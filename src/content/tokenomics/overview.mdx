...
![WELSH Corgi giving a whiteboard presentation](/trading-floor-lb.png)
...
Calling the `street-token` `mint()` function via the `emission-controller` contract. On the frontend, emission minting is referred to as mining, only to distinguish it from minting STREET and NFTs during the genesis event.
