...
The WELSH STREET Exchange implements a **constant product Automated Market Maker (AMM)** with integrated fee distribution to liquidity providers. The swap functions `swap-a-b` and `swap-b-a` enable bidirectional token swapping between WELSH Corgi Coin (WELSH) and STREET Token (STREET) using the mathematical formula `x * y = k`, where liquidity is preserved through constant product mechanics.

## Contract Architecture

The exchange maintains dual reserve pools and implements a **fee system** where all fees go directly to liquidity providers:

- **Swap Fees**: All trading fees are transferred to the rewards contract and distributed to liquidity providers
- **No Protocol Revenue**: The exchange has no income stream; all fees benefit liquidity providers
...
**Fee Distribution:**
- **Fee Component (`fee-a`)**: `(amount-a * fee) / BASIS` → All fees sent to rewards contract for liquidity provider distribution
...
2. AMM mechanics remain mathematically sound
...
- All fees benefit liquidity providers only
...
- Immutable 1% fee ensures sustainable rewards
- All fees distributed to liquidity providers
...
;; Expected behavior:
;; 1. Deducts 1% fee (10 WELSH tokens)
;; 2. Uses 990 WELSH for constant product calculation
;; 3. Returns calculated STREET tokens to user
;; 4. Distributes 10 WELSH to liquidity providers via rewards contract
```
...
;; Expected behavior:
;; 1. Deducts 1% fee (1,000 STREET tokens)
;; 2. Uses 99,000 STREET for constant product calculation
;; 3. Returns calculated WELSH tokens to user
;; 4. Distributes 1,000 STREET to liquidity providers via rewards contract
```
...
**Security**: These funds are permanently allocated to LP distributions and cannot be accessed by the contract owner.
