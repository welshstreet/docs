...
In the Meme Rewards Protocol, the CREDIT token is mathematically tied to the rewards accounting system.  Therefore ever transfer of the CREDIT token must update the rewards tracking for the sender and the recipient. 

Therefore, authorizations are placed on the `transfer` function which makes normal in-wallet transfers fail.  In order to transfer CREDIT tokens from wallet to wallet, or address to address, user's must utilize the [credit controller](/docs/core/controllers#credit-controller) function or the UI found on [Welsh Street](https://app.welshstreet.com/transfer).
...
- **Contract Link**: [credit-token](https://explorer.hiro.so/token/SP2A7PT3DMWVT2BHHMKQ2T0Y4AA05NST96SDM2FCE.credit-token?chain=mainnet)