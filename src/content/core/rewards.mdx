...
- ✅ Keep providing liquidity while claiming distributions
- ✅ Compound your position without losing accumulated distributions
- ✅ Maintain your market exposure during distribution claiming
...
- ✅ **`initial-liquidity`**: Owner gains LP tokens → reward position established
...
<Callout type="info">Always claim rewards before changing liquidity positions</Callout>
...
(define-public (claim-rewards)
  (let (
    (balance-lp (unwrap! (contract-call? .CREDIT get-balance tx-sender) ERR_BALANCENOT_AVAILABLE))
    (info (default-to {
...
(define-public (update-user-rewards (user principal))
  (let (
    (current-lp-balance (unwrap! (contract-call? .CREDIT get-balance user) ERR_BALANCENOT_AVAILABLE))
