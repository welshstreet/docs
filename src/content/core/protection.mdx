...
- Client / API (tx building): The wallet or app constructs the transaction and attaches post-conditions using Stacks SDK helpers. It broadcasts via the Stacks node API.
...
In short: Clarity executes the swap; the Stacks runtime enforces the promises a transaction makes about asset movements.
...
- MEV Sandwiching:
  If front/back-running pushes price outside a user’s declared bounds, the user’s min‑out / max‑in conditions fail. No toxic fill.
...
- Reasonable Fee Limits:
  Cap what the rewards contract can collect per tx. If a misconfig would exceed that cap, the tx reverts.


### Example 1 — Massive Move

> A whale submits a huge swap that is in the mempool, but the user’s quote is based on the previous state. The user submits a trade right after.

- The user’s tx post-conditions (attached by the user’s wallet):
  - Spend ≤ 1,000 A (`LessEqual` on the user’s standard principal for token A)
...
1. Whale confirms first → price moves down.
  Re-evaluating the user’s tx against the new pool state yields only 430 B for the 1,000 A input.
  - The “pool sends ≥ 480 B” condition fails → transaction reverts.
  - The user keeps token A, pays no fees, and receives no partial fills.

2. Whale confirms after the user (or not at all).
  The current state still supports ≥ 480 B out.
  - Both conditions pass → transaction executes and the user receives at least the promised amount.

<Callout type="default">The user never receives a worse-than-agreed price, even if state changes between signing and confirmation.</Callout>

### Example 2 — Anti-Windfall

> Due to a sudden oracle misread off-chain or a pending arbitrage, a quote could—momentarily—suggest an outsized payout.
...
- User tx includes a fee-limit post-condition against the rewards contract principal:
  "Rewards contract may receive ≤ F units for this transaction."
...
<Callout type="info">Post-conditions and Slippage help keep our users and liquidity pool safe </Callout>
