...
### Exchange Contract
- Contract: `exchange.clar` (or `street-market.clar` in devnet)
- Functions: 11 (liquidity operations, swaps, read-only queries)
- Immutable: 1% swap fee, 1% exit tax (all fees benefit liquidity providers)
- Error codes: 7 (initialization, amounts, authorization, calculations)
- Test files: Liquidity operations, swaps, read-only, scenarios
...
**`initial-liquidity.test.ts` (5 tests)**
...
**Note:** Fee and tax parameters are immutable (1% each). No parameter-setting functions exist in the contract.
...
The unit tests can be run by using `npm`. Make sure `npm` is installed on the system. In the file `./tests/vitestconfig.ts` the `disp` parameter may be set to `true` to view the test logs in the console.
